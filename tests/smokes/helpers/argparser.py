...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser = argparse.ArgumentParser(
        description="Run smoke tests for the nuc-kserve chart."
    )
    parser.add_argument("--chart-dir", default=str(repo_root), help="Path to the chart repository root.")
    parser.add_argument("--release-name", default="smoke", help="Release name used for rendered manifests.")
    parser.add_argument("--namespace", default="smoke", help="Namespace used for rendered namespaced resources.")
    parser.add_argument("--scenario", action="append", choices=SCENARIO_CHOICES, help="Scenario to run. May be specified multiple times. Defaults to all.")
